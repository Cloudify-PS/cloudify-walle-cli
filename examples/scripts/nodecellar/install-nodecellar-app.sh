...
script
    ...
    exec ${NODEJS_BINARIES_PATH}/bin/node ${NODECELLAR_SOURCE_PATH}/server.js 2>&1
end script\n" >> ${TEMP_DIR}/nodecellar.conf

init-checkconf -d ${TEMP_DIR}/nodecellar.conf

sudo cp ${TEMP_DIR}/nodecellar.conf /etc/init/nodecellar.conf
...
ctx logger info "Nodecellar runs as service"
